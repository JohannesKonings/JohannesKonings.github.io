...
- Updated [websites/tanstack/src/routes/\_\_root.tsx](websites/tanstack/src/routes/__root.tsx): Wrapped app in `ThemeProvider`; added `ThemeSync` component that runs `useEffect` to set `document.documentElement.classList.toggle("dark", theme === "dark")` when `theme` changes.
...
- **Lint:** No errors on changed files.
- **React Doctor:** 2 minor warnings (script defer in \_\_root; setState in BlogPostCard useEffect).
