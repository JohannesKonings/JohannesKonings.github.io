...
- **Content sync**: The app auto-runs content sync as part of `dev`, `build`, and `start`. This copies markdown from root `_posts/` and `_notes/` into `src/content/`, so no manual sync step is needed.
